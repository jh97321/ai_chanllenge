...
class EUG():
    ...
    def get_feature(self, dataset):
        ...
        features,_ = extract_features(self.model, dataloader, for_eval=True)
...
"""
def updata_lable(dataset, name, sample='random', load_path='random_split/', seed=0):
    np.random.seed(seed)
    random.seed(seed)
    ...
    if sample == 'random':
        ...
        for i, (f, pid, camid) in enumerate(dataset.trainval):
            if pid == 0:
                continue
            if pid not in list(cluster.keys()):
                cluster[pid] = []
            cluster[pid].append([osp.join(dataset.images_dir, f), pid, camid])
