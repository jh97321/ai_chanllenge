...
class Trainer(BaseTrainer):
    ...
    def _forward(self, inputs, targets, epoch):
        ...
        #new added by wc
        # x1 triplet loss
        loss_tri, prec_tri = self.criterions[0](outputs[1], targets, epoch)
        # x2 global feature cross entropy loss
        #loss_global = self.criterions[1](outputs[1], targets)
        loss_global, prec_global = self.criterions[1](outputs[2], targets, epoch)
...
class FinedTrainer2(object):
    ...
    def _forward(self, inputs, pids, epoch):
        loss = 0.0
        outputs = self.model(*inputs)
        loss_global, prec_global = self.criterions[1](outputs[2], pids[0], epoch)
        loss = loss_global
        prec = prec_global
        # outputs_split = [outputs[0][0], torch.cat(outputs[0][1:], dim=1)]
        if isinstance(outputs[1], list):
            for i, output_p in enumerate(outputs[1]):
                loss_tri, prec_tri = self.criterions[0](output_p, pids[i], epoch)
                loss += loss_tri
        else:
            loss_tri, prec_tri = self.criterions[0](outputs[1], pids[0], epoch)
            loss += loss_tri
        """ if len(outputs) == 3:
...
class JointTrainer2(object):
    ...
    def train(self, epoch, eug_loader, optimizer, print_freq=10):
        self.model.train()
        batch_time = AverageMeter()
        data_time = AverageMeter()
        losses = AverageMeter()
        ...
        for i, inputs_eug in enumerate(eug_loader):
            data_time.update(time.time() - end)
            # load source_triplet and target
            """ try:
                inputs_eug = next(eug_loader_iter)
            ...
            loss, prec = self._forward(inputs_eug, pids_eug, epoch)

            losses.update(loss.item(), inputs_eug[0].size(0))
            # print(pids[0].size())
            # print(pids_eug.size())
            precisions.update(prec, pids_eug.size(0) + pids_eug.size(0))
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            ...
            if (i + 1) % print_freq == 0:
                print('Epoch: [{}][{}/{}]\t'
                      ...
                      'Loss {:.3f} ({:.3f})\t'
                      'Prec {:.2%} ({:.2%})\t'
                      .format(epoch, i + 1, len(eug_loader),
                              batch_time.val, batch_time.avg,
                              data_time.val, data_time.avg,
                              losses.val, losses.avg,
                              precisions.val, precisions.avg))
    ...
    def _forward(self, inputs_eug, pids_eug, epoch, w_eug=None):
        loss_uns = 0.0
        loss_os = 0.0
        """ outputs = self.model(*inputs)
        loss_global, prec_global = self.criterions[1](outputs[2], pids[0], epoch)
        ...
        outputs_eug = self.model(*inputs_eug)
        loss_global_eug, prec_global_eug = self.criterions[1](outputs_eug[3], pids_eug, epoch)
        loss_os += loss_global_eug
        prec = prec_global_eug
        if isinstance(outputs_eug[1], list):
            for i, output_p in enumerate(outputs_eug[1]):
            ...
            loss_tri, prec_tri = self.criterions[0](outputs_eug[1], pids_eug, epoch)
            loss_os += loss_tri
        
        """ if isinstance(outputs_eug[2], list):
            for i, output_p in enumerate(outputs_eug[2]):
                loss_cro = self.criterions[2](output_p, pids_eug)
                loss_os += loss_cro
            # loss_tri, prec_tri = self.criterions[0](outputs_eug[0][0], pids_eug, epoch)
            # loss_os += loss_tri
        else:
            loss_cro = self.criterions[2](outputs_eug[2], pids_eug)
            loss_os += loss_cro
        
        if isinstance(outputs_eug[1], list):
            for i, output_p in enumerate(outputs_eug[1]):
                loss_cen = self.criterions[3](output_p, pids_eug)
                loss_os += loss_cen
            # loss_tri, prec_tri = self.criterions[0](outputs_eug[0][0], pids_eug, epoch)
            # loss_os += loss_tri
        else:
            loss_cen = self.criterions[3](outputs_eug[1], pids_eug)
            loss_os += loss_cen """
