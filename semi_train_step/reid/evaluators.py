...
def extract_features(model, data_loader, print_freq=20, for_eval=False, metric=None):
    model.eval()
...
class Evaluator(object):
    ...
    def evaluate(self, data_loader, query, gallery, metric=None):
        features, _ = extract_features(self.model, data_loader, print_freq=self.print_freq, for_eval=True)
        distmat = pairwise_distance(features, query, gallery, metric=metric)
        return evaluate_all(distmat, query=query, gallery=gallery)

    def evaluate_same_cams(self, data_loader, query, gallery, metric=None):
        features, _ = extract_features(self.model, data_loader, for_eval=True)
